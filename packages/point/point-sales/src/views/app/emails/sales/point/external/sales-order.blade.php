...
<body>
    <div class="invoice-box">
        Hello Mr/Mrs/Ms/ {{ $sales_order->person->name }},<br/>
        You have an email sales order from <br>
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            @foreach($sales_order->items as $items)
                <tr class="item">
            ...
            <tr>
                ...
                <td align="right">{{ number_format_quantity($sales_order->subtotal) }}</td>
            </tr>
            <tr>
                <td colspan="5" align="right">Discount (%)</td>
                <td align="right">{{ number_format_quantity($sales_order->discount) }}</td>
            </tr>
            <tr>
                <td colspan="5" align="right">Tax Base</td>
                <td align="right">{{ number_format_quantity($sales_order->tax_base) }}</td>
            </tr>
            @if($sales_order->type_of_tax != 'non')
            <tr>
                <td colspan="5" align="right">Tax ({{ $sales_order->type_of_tax }})
                </td>
                <td align="right">{{ number_format_quantity($sales_order->tax) }}</td>
            </tr>
            @endif
            <tr>
                <td colspan="5" align="right">Expedition Fee</td>
                <td align="right">{{ number_format_quantity($sales_order->expedition_fee) }}</td>
            </tr>
            <tr>
                <td colspan="5" align="right">Total</td>
                <td align="right">{{ number_format_quantity($sales_order->total) }}</td>
            </tr>
        </table>
    </div>
</body>
