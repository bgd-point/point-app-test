...
@section('content')
    <div id="page-content">
        ...
        <form action="{{url('sales/point/indirect/downpayment/send-request-approval')}}" method="post">
            {!! csrf_field() !!}
                        ...
                        <table class="table table-striped table-bordered">
                            ...
                            @foreach($list_downpayment as $downpayment)
                                <tr id="list-{{$downpayment->formulir_id}}">
                                    ...
                                    <td>
                                        <a href="{{ url('master/contact/person/'.$downpayment->person_id) }}">{{ $downpayment->person->codeName}}</a>
                                    </td>
                                    <td>{{ number_format_quantity($downpayment->amount) }}</td>
                                    <td>{{ $downpayment->formulir->approvalTo->name}}</td>
                                    <td>
