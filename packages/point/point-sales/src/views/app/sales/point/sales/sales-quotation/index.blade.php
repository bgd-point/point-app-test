    ...
    <div id="page-content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="{{ url('sales/point/indirect/sales-quotation') }}" method="get" class="form-horizontal">
                ...
                <div class="table-responsive">
                    ...
                    ?>
                    {!! $list_sales_quotation->appends(['order_by'=>app('request')->get('order_by'), 'order_type'=>app('request')->get('order_type'), 'status'=>app('request')->get('status'), 'search'=>app('request')->get('search'), 'date_from'=>app('request')->get('date_from'), 'date_to'=>app('request')->get('date_to') ])->render() !!}
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($list_sales_quotation as $sales_quotation)
                            <tr>
                                <td>
                                    @include('framework::app.include._approval_status_label', ['approval_status' => $sales_quotation->formulir->approval_status])
                            ...
                            @foreach($sales_quotation->items as $detail)
                                <tr>
                                    ...
                                    <td>{{number_format_price($detail->quantity * $detail->price ) }} </td>
                                </tr>
                            @endforeach
                        @endforeach
                        </tbody>
...
function selectData(order_by, order_type) {
    ...
    var url = '{{url()}}/sales/point/indirect/sales-quotation/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
    location.href = url;
}
</script>
@stop
