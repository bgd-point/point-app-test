    ...
    <div id="page-content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="{{url('sales/point/service/payment-collection/'.$payment_collection->id.'/edit-review')}}"
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <input type="text"
                                                                   ...
                                                                   onkeyup="updateDp()" value="{{$refer_to_amount}}"/>
                                            ...
                                            <table id="item-datatable" class="table table-striped">
                                                ...
                                                @foreach($payment_collection->others as $payment_collection_other)
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <select id="allocation-id-{{$counter}}" name="allocation_id[]"
                                                                    class="selectize" style="width: 100%;"
                                                                    data-placeholder="Choose one..">
                                                                @foreach($list_allocation as $allocation)
                                                                    <option value="{{$allocation->id}}" @if($allocation->id == $payment_collection_other->allocation_id) selected @endif>{{$allocation->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                    </tr>
                                    ...
                                    @foreach($list_user_approval as $user_approval)
                                        @if($user_approval->may('approval.point.sales.payment.collection'))
                                            <option value="{{$user_approval->id}}"
                                                    @if($payment_collection->formulir->approval_to == $user_approval->id) selected @endif>{{$user_approval->name}}</option>
                                        @endif
    ...
    <script>
        ...
        $('#addItemRow').on('click', function () {
            item_table.row.add([
                ...
                '<input type="text" id="total-' + counter + '" name="total[]" class="form-control text-right format-price-alt row-total calculate" value="0" />',
