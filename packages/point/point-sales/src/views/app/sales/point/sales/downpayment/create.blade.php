            ...
            <div class="panel-body">
                <form action="{{url('sales/point/indirect/downpayment')}}" method="post" class="form-horizontal form-bordered">
                    ...
                    @if($sales_order !== "")
                            ...
                            <div class="form-group">
                                <div class="col-md-12">
                                    <legend><i class="fa fa-angle-right"></i> REF
                                        <a target="_blank"
                                           href="{{ url('sales/point/indirect/sales-order/'.$sales_order->id) }}">#{{$sales_order->formulir->form_number}}</a>
                                    </legend>
                                    <input type="hidden" name="order_reference" class="form-control" value="{{$sales_order->id}}">
                                </div>
                            </div>
                        </fieldset>
                        <div class="form-group">
                            ...
                            <div class="col-md-6 content-show">
                                {{ date_format_view($sales_order->formulir->form_date) }}
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label class="col-md-3 control-label">Total Remaining Amount Sales</label>
                            <div class="col-md-6 content-show">
                                {{ number_format_price($sales_order->total - $sales_order->getTotalGoodsSent($sales_order->id)) }}
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-3 control-label">Total Remaining Downpayment</label>
                            <div class="col-md-6 content-show">
                                {{ number_format_price($sales_order->getTotalRemainingDownpayment($sales_order->id)) }}
                            </div>
                        </div>
                    @endif
