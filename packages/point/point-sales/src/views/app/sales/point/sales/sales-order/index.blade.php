    ...
    <div id="page-content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="{{ url('sales/point/indirect/sales-order') }}" method="get" class="form-horizontal">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($list_sales_order as $sales_order)
                            ...
                            <td>{{number_format_price($sales_order->total)}}
                                <i class='text-info' style='font-size:12px'> [{{ $sales_order->is_cash == 1 ? 'Cash' : 'Credit' }}]</i> <br>
                            </td>
                            <td>{{ number_format_price($sales_order->getTotalRemainingDownpayment(($sales_order->id))) }}</td>
                            <td>{{ number_format_price($sales_order->getTotalDownpayment(($sales_order->id))) }}</td>
                            <td>
                                @include('framework::app.include._approval_status_label', ['approval_status' => $sales_order->formulir->approval_status])
...
function selectData(order_by, order_type) {
    ...
    var url = '{{url()}}/sales/point/indirect/sales-order/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
    location.href = url;
}
</script>
@stop
