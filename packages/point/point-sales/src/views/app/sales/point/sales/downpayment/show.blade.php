                                ...
                                <div class="form-group">
                                    <div class="col-md-12">
                                        <legend><i class="fa fa-angle-right"></i> REF#
                                            @if($downpayment->sales_order_id !== null && $downpayment->sales_order_id > 0)
                                                <a target="_blank" href="{{ url('sales/point/indirect/sales-order/'.$downpayment->sales_order_id) }}">{{$downpayment->salesOrder->formulir->form_number}}</a>@endif
                                        </legend>
                                    </div>
                                </div>
                ...
                <div class="tab-pane" id="block-tabs-settings">
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                @if(formulir_view_edit($downpayment->formulir, 'update.point.sales.downpayment'))
                                    <a href="{{url('sales/point/indirect/downpayment/'.$downpayment->id.'/edit')}}"
                                       class="btn btn-effect-ripple btn-info"><i class="fa fa-pencil"></i> Edit</a>
                                @endif

                                @if(formulir_view_cancel_or_request_cancel($downpayment->formulir, 'delete.point.sales.downpayment', 'approval.point.sales.downpayment') == 1)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureCancelForm('{{url('formulir/cancel')}}', '{{ $downpayment->formulir_id }}','approval.point.sales.downpayment')">
                                        <i class="fa fa-times"></i> 
                                        Cancel Form
                                    </a>
                                @elseif(formulir_view_cancel_or_request_cancel($downpayment->formulir, 'delete.point.sales.downpayment', 'approval.point.sales.downpayment') == 2)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureRequestCancelForm(this, '{{url('formulir/requestCancel')}}', '{{ $downpayment->formulir_id }}', 'delete.point.sales.downpayment')">
                                        <i class="fa fa-times"></i> 
                                        Request Cancel Form
                                    </a>
                                @endif
                    ...
                    @if(formulir_view_approval($downpayment->formulir, 'approval.point.sales.downpayment'))
                        <fieldset>
                            ...
                            <div class="form-group">
                                <div class="col-md-6">
                                    <form action="{{url('sales/point/indirect/downpayment/'.$downpayment->id.'/approve')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                ...
                                <div class="col-md-6">
                                    <form action="{{url('sales/point/indirect/downpayment/'.$downpayment->id.'/reject')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                        <div class="input-group">
                                            <input type="text" name="approval_message" class="form-control"
                                            ...
                                            @foreach($list_downpayment_archived as $downpayment_archived)
                                                <tr>
                                                    <td class="text-center">
                                                        <a href="{{ url('sales/point/indirect/downpayment/'.$downpayment_archived->id.'/archived') }}"
                                                           data-toggle="tooltip"
                                                           title="Show"
