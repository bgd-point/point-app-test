    ...
    <div id="page-content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="{{ url('sales/point/service/payment-collection') }}" method="get"
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($list_payment_collection as $payment_collection)
                            ...
                            <?php array_push($array_payment_collection_id,$payment_collection->id); ?>
                                ...
                                <td>
                                    <a href="{{ url('sales/point/service/payment-collection/'.$payment_collection->id) }}">{{ $payment_collection->formulir->form_number}}</a>
                                </td>
                                <td>
...
function showAll(){
    ...
    if(check_show == 0){
        ...
        for (var x = temp.length - 1; x >= 0; x--) {
            var str_url = "{{ url('sales/point/service/payment-collection/detail/') }}/"+temp[x];
            $.ajax({ url:str_url, success: function(data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    $('#row_detail_'+data[i].point_sales_service_payment_collection_id).after(extend_table_row);
...
function selectData(order_by, order_type) {
    ...
    var url = '{{url()}}/sales/point/service/payment-collection/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
    location.href = url;
}
</script>
@stop
