        ...
        \Queue::push(function ($job) use ($request, $fileName, $storage) {
            \Excel::create($fileName, function ($excel) use ($storage, $request) {
                ...
                $excel->sheet('Data', function ($sheet) use ($request) {
                    ...
                    $date_from = date_format_view($request['date_from'] ?: date('Y-m-01 00:00:00'));
                    $date_to   = date_format_view($request['date_to']   ?: date('Y-m-d 23:59:59'));
                    
                    $sheet->setCellValue('B3', '(' . $date_from . ')');
                    $sheet->setCellValue('D3', '(' . $date_from . ')-(' . $date_to . ')');
                    $sheet->setCellValue('F3', '(' . $date_from . ')-(' . $date_to . ')');
                    $sheet->setCellValue('H3', '(' . $date_to . ')');
                    ...
                    $list_report = Inventory::joinItem()
                        ->groupBy('inventory.item_id')
                        ->where('inventory.total_quantity', '>', 0)
                        // ->where(function ($query) use ($array_of_search, $warehouse, $date_from, $date_to) {
                        //     if ($warehouse > 0) {
