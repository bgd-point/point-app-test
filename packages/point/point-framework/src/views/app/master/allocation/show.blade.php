    ...
    <div class="block full">
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="block-tabs-form">
                <div class="form-horizontal form-bordered">
                    <div class="form-group">
                        ...
                        <div class="col-md-6 content-show">{{$allocation->name}}</div>
                    </div> 
                </div>
            </div>
            ...
            <div class="tab-pane" id="block-tabs-settings">
                <a href="{{url('master/allocation/'.$allocation->id.'/edit')}}" class="btn btn-effect-ripple btn-info"><i class="fa fa-pencil"></i> Edit</a>
                <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureDelete({{$allocation->id}}, '{{url('master/allocation/delete')}}', '/master/allocation')"><i class="fa fa-times"></i> Delete</a>

                <a id="link-state-{{$allocation->id}}" href="javascript:void(0)" 
                class="btn btn-effect-ripple {{$allocation->disabled == 0 ? 'btn-success' : 'btn-default' }}" 
                onclick="state({{$allocation->id}})">
                <i id="icon-state-{{$allocation->id}}" class="{{$allocation->disabled == 0 ? 'fa fa-pause' : 'fa fa-play' }}"></i> {{$allocation->disabled == 0 ? 'disable' : 'enable' }}</a>
            </div>
        </div>
        <!-- END Tabs Content -->
    </div> 
