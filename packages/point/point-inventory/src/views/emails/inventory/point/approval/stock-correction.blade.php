    ...
    <div class="invoice-box">
            ...
            <table cellpadding="0" cellspacing="0" style="padding: 20px 0;">
                ...
                <tr>
                    ...
                    <td>
                        {{ \DateHelper::formatView($stock_correction->formulir->form_date) }}
                    </td>
                </tr>
                <tr>
                    <td style="width: 20%">
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                @foreach($stock_correction->items as $items)
                    <tr class="item">
                        <td align="center">
                            {{ $items->item->name }}
                        </td>
                        <td align="right">
                            {{ $items->stock_in_database }} {{ $items->unit }}
                        </td>
                        <td align="right">
                            {{ $items->quantity_correction}} {{ $items->unit }}
                        </td>
                        <td align="center">
                ...
                <tr>
                    <td colspan="4">
                       <a href="{{ $url . '/formulir/'.$stock_correction->formulir_id.'/approval/check-status/'.$token }}"> 
                       <input type="button" class="btn btn-check" value="CHECK">
                       </a>
                       <a href="{{ $url . '/inventory/point/stock-correction/'.$stock_correction->id.'/approve?token='.$token }}"> 
                       <input type="button" class="btn btn-success" value="APPROVE">
                       </a>
                       <a href="{{ $url . '/inventory/point/stock-correction/'.$stock_correction->id.'/reject?token='.$token }}">
        ...
        <br>
        <a href="{{ $url . '/inventory/point/stock-correction/approve-all/?formulir_id='.$array_formulir_id.'&token='.$token }}">
            <input type="button" class="btn btn-primary" value="Approve All">
        </a>
        <a href="{{ $url . '/inventory/point/stock-correction/reject-all/?formulir_id='.$array_formulir_id.'&token='.$token }}">
