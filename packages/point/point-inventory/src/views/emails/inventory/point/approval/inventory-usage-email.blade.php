...
<body>
    ...
    <div class="invoice-box">
        ...
        @foreach($list_data as $inventory_usage)
            <table cellpadding="0" cellspacing="0" style="padding: 20px 0;">
                ...
                <tr>
                    ...
                    <td>
                        {{ \DateHelper::formatView($inventory_usage->formulir->form_date) }}
                    </td>
                </tr>
                <tr>
                    <td style="width: 20%">
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                @foreach($inventory_usage->listInventoryUsage as $items)
                    <tr class="item">
                        <td align="center">
                            {{ $items->item->name }}
                        </td>
                        ...
                        <td align="right">
                            {{ $items->quantity_usage }}
                        </td>
                        <td align="center">
                            {{ $items->usage_notes }}
                        </td>
                        <td align="center">
                            {{ $items->allocation->name }}
                        </td>
                    </tr>
                @endforeach
                ...
                <tr>
                    <td colspan="4">
                       <a href="{{ $url . '/formulir/'.$inventory_usage->formulir_id.'/approval/check-status/'.$token }}"> 
                       <input type="button" class="btn btn-check" value="CHECK">
                       </a>
                       <a href="{{ $url . '/inventory/point/inventory-usage/'.$inventory_usage->id.'/approve?token='.$token }}"> 
                       <input type="button" class="btn btn-success" value="APPROVE">
                       </a>
                       <a href="{{ $url . '/inventory/point/inventory-usage/'.$inventory_usage->id.'/reject?token='.$token }}">
        ...
        <br>
        <a href="{{ $url . '/inventory/point/inventory-usage/approve-all/?formulir_id='.$array_formulir_id.'&token='.$token }}">
            <input type="button" class="btn btn-primary" value="Approve All">
        </a>
        <a href="{{ $url . '/inventory/point/inventory-usage/reject-all/?formulir_id='.$array_formulir_id.'&token='.$token }}">
