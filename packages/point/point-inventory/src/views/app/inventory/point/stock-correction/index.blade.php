...
<div id="page-content">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{ url('inventory/point/stock-correction') }}" method="get" class="form-horizontal">
            ...
            <div class="table-responsive">
                ...
                ?>
                {!! $list_stock_correction->appends(['order_by'=>app('request')->get('order_by'), 'order_type'=>app('request')->get('order_type'), 'status'=>app('request')->get('status'), 'search'=>app('request')->get('search'), 'date_from'=>app('request')->get('date_from'), 'date_to'=>app('request')->get('date_to') ])->render() !!}
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @include('framework::app.include._approval_status_label', ['approval_status' => $stock_correction->formulir->approval_status])
                        ...
                        @foreach($stock_correction->items as $correctionItem)
                            <tr>
                                <td></td>
                                <td>{{$correctionItem->item->name}}</td>
                                <td>{{$correctionItem->stock_in_database}} {{$correctionItem->unit}}</td>
                                <td>{{$correctionItem->quantity_correction}} {{$correctionItem->unit}}</td>
                            </tr>
                        @endforeach
...
function selectData(order_by, order_type) {
    ...
    var url = '{{url()}}/inventory/point/stock-correction/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
    location.href = url;
}
</script>
@stop
