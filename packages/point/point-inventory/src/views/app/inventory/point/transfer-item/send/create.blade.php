...
<div id="page-content">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{url('inventory/point/transfer-item/send')}}" method="post"
            ...
            <div class="form-group">
                ...
                <div class="col-md-3">
                    <input disabled type="text" id="form_date" name="form_date" class="form-control date input-datepicker" data-date-format="{{date_format_masking()}}" placeholder="{{date_format_masking()}}" value="{{ date(date_format_get(), strtotime(\Carbon::now())) }}">
                </div>
                <div class="col-md-3">
            ...
            <div class="form-group">
                ...
                <div class="col-md-6">
                    <div id="content-warehouse">
                        <select id="warehouse-to" name="warehouse_to" class="selectize" style="width: 100%;" data-placeholder="Choose one..">
                            <option ></option>
                            @foreach($list_warehouse as $warehouse)
                                <option value="{{$warehouse->id}}" @if(old('warehouse_to')==$warehouse->id) selected @endif> {{$warehouse->codeName}} </option>
                            @endforeach
                        </select>
                    </div>
                                ...
                                <tbody class="manipulate-row">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if(old('item_id.'.$counter))
                                            <option value="{{old('item_id.'.$counter)}}">{{Point\Framework\Models\Master\Item::find(old('item_id.'.$counter))->codeName}}</option>
                                            @endif
                                            </select>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="input-group">
                                                <input type="text" name="qty_send[]" class="form-control text-right" value="{{ old('qty_send.'.$counter) }}" />
...
@section('scripts')
<script>
var counter = {!! json_encode($counter) !!};
...
$('#addItemRow').on( 'click', function () {
    ...
    reloadItemInSelectize("#item-id-"+counter, true,);
    counter++;
} );
...
function reloadItemInSelectize(item_id, reset) {
    $.ajax({
    ...
    if( counter > 0 ) {
        for (var i = 0; i < counter; i++) {
            if($('#item-id-'+i).length != 0){
                reloadItemInSelectize('#item-id-'+i, false);   
            }
        }
    }
});
