...
class StockCorrectionHelper
{
    ...
    public static function approve($stock_correction)
    {
        foreach ($stock_correction->items as $stock_correction_item) {
            ...
            $inventory->form_date = $stock_correction->formulir->form_date;
            ...
            $inventory->price =  InventoryHelper::getCostOfSales($stock_correction->formulir->form_date, $stock_correction_item->item_id, $stock_correction->warehouse_id);
    ...
    public static function updateJournal($stock_correction)
    {
        ...
        foreach ($stock_correction->items as $stock_correction_item) {
            ...
            $cost_of_sales = InventoryHelper::getCostOfSales($stock_correction->formulir->form_date, $stock_correction_item->item_id, $stock_correction->warehouse_id);
            $cost_of_sales = $cost_of_sales * $stock_correction_item->quantity_correction;

            $journal = new Journal();
            $journal->form_date = $stock_correction->formulir->form_date;
