...
<div id="page-content">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{ url('facility/bumi-shares/sell') }}" method="get" class="form-horizontal">
            ...
            @if($list_shares_sell->count() > 0)
            <div class="table-responsive">
                {!! $list_shares_sell->appends(['order_by'=>app('request')->get('order_by'), 'order_type'=>app('request')->get('order_type'), 'status'=>app('request')->get('status'), 'search'=>app('request')->get('search'), 'date_from'=>app('request')->get('date_from'), 'date_to'=>app('request')->get('date_to') ])->render() !!}
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr id="list-{{$shares_sell->formulir_id}}">
                            ...
                            <td><a href="{{ url('facility/bumi-shares/broker/'.$shares_sell->broker_id) }}">{{ $shares_sell->broker->name}}</a></td>
                            <td><a href="{{ url('facility/bumi-shares/shares/'.$shares_sell->shares_id) }}">{{ $shares_sell->shares->name}}</a></td>
                            <td>{{ number_format_quantity($shares_sell->quantity) }}</td>
                            <td>{{ \NumberHelper::formatAccounting($shares_sell->price) }}</td>
                            <td>{{ number_format_quantity($shares_sell->fee) }}</td>
                            <td><a href="{{ url('facility/bumi-shares/owner/'.$shares_sell->owner_id) }}">{{ $shares_sell->owner->name}}</a></td>
                            <td><a href="{{ url('facility/bumi-shares/owner-group/'.$shares_sell->owner_group_id) }}">{{ $shares_sell->ownerGroup->name}}</a></td>
                            <td>
...
function selectData(order_by, order_type) {
    ...
    var url = '{{url()}}/facility/bumi-shares/sell?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
    location.href = url;
}
</script>
@stop
