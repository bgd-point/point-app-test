...
<page size="A5" layout="landscape">
    ...
    @for($i=0;$i<$number_of_table;$i++)
        ...
        <table width="100%" class="table">
            ...
            <tbody>
            <tr>
                <td colspan="2" class="column" id="text-center">BUKTI BANK {{$bank->payment_flow == 'out' ? 'KELUAR' : 'MASUK'}}</td>
                <td colspan="1" class="column" style="border-right:0px" class="text-left">Tanggal</td>
                <td class="column" style="border-left:0px;">: {{ date_format_view($bank->formulir->form_date) }}</td>
            </tr>
            <tr>
                <td rowspan="2" class="column" colspan="2">{{$bank->payment_flow == 'out' ? 'dibayar kepada' : 'diterima dari'}} : {{ $bank->person->name }}</td>
                <td colspan="1" class="column" style="border-right:0px" class="text-left">Nomor</td>
                <td class="column" style="border-left:0px;">: {{ $bank->formulir->form_number }}</td>
            </tr>
            <tr>
                <td class="column">&nbsp;</td>
                <td class="column">&nbsp;</td>
            ...
            <tr>
                <td colspan="3" id="text-left" class="column">Terbilang :<b>{{number_to_text(abs($bank->total))}} Rupiah</b></td>
                <td id="text-right" class="column"><b>{{ number_format_price(abs($bank->total), 0) }}</b></td>
            </tr>
