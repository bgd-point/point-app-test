...
<div id="page-content">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{ url('finance/point/debt-cash') }}" method="get" class="form-horizontal">
            ...
            @if($list_cash->count() > 0)
            <?php 
                $order_by = \Input::get('order_by') ? : 0;
                $order_type = \Input::get('order_type') ? : 0;
            ?>
            <div class="table-responsive">
                {!! $list_cash->appends(['order_by'=>app('request')->get('order_by'), 'order_type'=>app('request')->get('order_type'), 'status'=>app('request')->get('status'), 'search'=>app('request')->get('search'), 'date_from'=>app('request')->get('date_from'), 'date_to'=>app('request')->get('date_to') ])->render() !!}
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        @foreach($list_cash as $cash)
                            <tr @if($cash->formulir->form_status == -1) style="background:red;color:white" @endif>
                                <td><a href="#" onclick="pagePrint('/finance/point/debt-cash/print/{{$cash->id}}');" data-toggle="tooltip" title="Print" class="btn btn-effect-ripple btn-xs btn-info"><i class="fa fa-print"></i> Print</a></td>
                                <td>{{ date_format_view($cash->formulir->form_date) }}</td>
                                <td>
                                    <a href="{{ url('finance/point/debt-cash/'. $cash->payment_flow .'/'.$cash->id) }}" @if($cash->formulir->form_status == -1) style="background:red;color:white !important" @endif>{{ $cash->formulir->form_number}}</a>
                                </td>
                                <td>{!! get_url_person($cash->person->id) !!}</td>
                                <td>{{ $cash->formulir->notes }}</td>
                                <td colspan="2" class="text-center">
                                    @if($cash->formulir->form_status == -1)
                            ...
                            @foreach($cash->detail as $cash_detail)
                                <tr @if($cash->formulir->form_status == -1) style="background:red;color:white" @endif>
                                    <td></td>
                                    <td></td>
                                    <td colspan="3">[{{ $cash_detail->coa->account }}] {{ $cash_detail->notes_detail }}</td>
                                    <td style="text-align: right">
                                        @if($cash->payment_flow == 'in')
                                        {{ number_format_price($cash_detail->amount) }}
                                        @endif
                                    </td>
                                    <td style="text-align: right">
                            ...
                            <td>&nbsp;</td>
                                ...
                                <td style="text-align: right">
                                    @if($cash->payment_flow == 'in')
                                    <b>{{ number_format_price($cash->total) }}</b>
                                    @endif
                                </td>
                                <td style="text-align: right">
                                    @if($cash->payment_flow == 'out')
                                    <b>{{ number_format_price(abs($cash->total)) }}</b>
                                    @endif
                                </td>
                            </tr>
                        @endforeach  
...
<script>
    ...
    function selectData(order_by, order_type) {
        ...
        var url = '{{url()}}/finance/point/debt-cash/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
        location.href = url;
    }
</script>
@stop
