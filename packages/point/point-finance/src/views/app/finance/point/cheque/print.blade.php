...
<page size="A5" layout="landscape">
    ...
    @for($i=0;$i<$number_of_table;$i++)
        ...
        <table width="100%" class="table">
            ...
            <tbody>
            <tr>
                <td colspan="2" class="column" id="text-center">BUKTI BANK {{$cheque->payment_flow == 'out' ? 'KELUAR' : 'MASUK'}}</td>
                <td colspan="1" class="column" style="border-right:0px" class="text-left">Tanggal</td>
                <td class="column" style="border-left:0px;">: {{ date_format_view($cheque->formulir->form_date) }}</td>
            </tr>
            <tr>
                <td rowspan="2" class="column" colspan="2">{{$cheque->payment_flow == 'out' ? 'dibayar kepada' : 'diterima dari'}} : {{ $cheque->person->name }}</td>
                <td colspan="1" class="column" style="border-right:0px" class="text-left">Nomor</td>
                <td class="column" style="border-left:0px;">: {{ $cheque->formulir->form_number }}</td>
            </tr>
            <tr>
                <td class="column">&nbsp;</td>
                <td class="column">&nbsp;</td>
            ...
            @for($j=$i*$max_row;$j<count($list_data);$j++)
                ...
                @if($count <= $limit)
                    ...
                    @if($count_word < 85)
                        ...
                        @if($note_detail_2)
                            <tr>
                                <td colspan="3" class="column" id="text-left">{{ $note_detail_2 }}</td>
            ...
            <tr>
                <td colspan="3" id="text-left" class="column">Terbilang :<b>{{number_to_text(abs($cheque->total))}} Rupiah</b></td>
                <td id="text-right" class="column"><b>{{ number_format_price(abs($cheque->total), 0) }}</b></td>
            </tr>
