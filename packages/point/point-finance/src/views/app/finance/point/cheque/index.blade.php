...
<div id="page-content">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{ url('finance/point/cheque') }}" method="get" class="form-horizontal">
            ...
            @if($list_cheque->count() > 0)
            <?php 
                $order_by = \Input::get('order_by') ? : 0;
                $order_type = \Input::get('order_type') ? : 0;
            ?>
            <div class="table-responsive">
                {!! $list_cheque->appends(['order_by'=>app('request')->get('order_by'), 'order_type'=>app('request')->get('order_type'), 'status'=>app('request')->get('status'), 'search'=>app('request')->get('search'), 'date_from'=>app('request')->get('date_from'), 'date_to'=>app('request')->get('date_to') ])->render() !!}
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                        @foreach($list_cheque as $cheque)
                            <tr @if($cheque->formulir->form_status == -1) style="background:red;color:white" @endif>
                                <td>
                                    <a href="#" onclick="pagePrint('/finance/point/cheque/print/{{$cheque->id}}');" data-toggle="tooltip" title="Print" class="btn btn-effect-ripple btn-xs btn-info"><i class="fa fa-print"></i> Print</a>
                                </td>
                                <td>{{ date_format_view($cheque->formulir->form_date) }}</td>
                                <td>
                                    <a href="{{ url('finance/point/cheque/'. $cheque->payment_flow .'/'.$cheque->id) }}" @if($cheque->formulir->form_status == -1) style="background:red;color:white !important" @endif>{{ $cheque->formulir->form_number}}</a>
                                </td>
                                <td>{!! get_url_person($cheque->person->id) !!}</td>
                                <td>{{ $cheque->formulir->notes }}</td>
                                <td colspan="2" class="text-center">
                                    @if($cheque->formulir->form_status == -1)
                            ...
                            @foreach($cheque->detail as $cheque_detail)
                                <tr @if($cheque->formulir->form_status == -1) style="background:red;color:white" @endif>
                                    <td></td>
                                    <td colspan="3">[{{ $cheque_detail->coa->account }}] {{ $cheque_detail->notes_detail }}</td>
                                    <td style="text-align: right" colspan="2">
                                        @if($cheque->payment_flow == 'in')
                                        {{ number_format_price($cheque_detail->amount) }}
                                        @endif
                                    </td>
                                    <td style="text-align: right" colspan="2">
                                        @if($cheque->payment_flow == 'out')
                                        {{ number_format_price($cheque_detail->amount) }}
                                        @endif
                                    </td>
                                </tr>
                            ...
                            <tr @if($cheque->formulir->form_status == -1) style="background:red;color:white" @endif>
                                ...
                                <td style="text-align: right" colspan="2">
                                    @if($cheque->payment_flow == 'in')
                                    <b>{{ number_format_price($cheque->total) }}</b>
                                    @endif
                                </td>
                                <td style="text-align: right" colspan="2">
                                    @if($cheque->payment_flow == 'out')
                                    <b>{{ number_format_price(abs($cheque->total)) }}</b>
                                    @endif
                                </td>
                            </tr>
                        @endforeach  
...
<script>
    ...
    function selectData(order_by, order_type) {
        ...
        var url = '{{url()}}/finance/point/cheque/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
        location.href = url;
    }
</script>
@stop
