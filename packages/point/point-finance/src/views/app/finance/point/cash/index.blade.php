...
<div id="page-content">
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form action="{{ url('finance/point/cash') }}" method="get" class="form-horizontal">
                    ...
                    <tbody>
                        @foreach($list_cash as $cash)
                            <tr @if($cash->formulir->form_status == -1) style="background:red;color:white" @endif>
                                <td><a href="#" onclick="pagePrint('/finance/point/cash/print/{{$cash->id}}');" data-toggle="tooltip" title="Print" class="btn btn-effect-ripple btn-xs btn-info"><i class="fa fa-print"></i> Print</a></td>
                                <td>{{ date_format_view($cash->formulir->form_date) }}</td>
                                <td>
                                    <a href="{{ url('finance/point/cash/'. $cash->payment_flow .'/'.$cash->id) }}" @if($cash->formulir->form_status == -1) style="background:red;color:white !important" @endif>{{ $cash->formulir->form_number}}</a>
                                </td>
...
<script>
    ...
    function selectData(order_by, order_type) {
        ...
        var url = '{{url()}}/finance/point/cash/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
        location.href = url;
    }
</script>
@stop
