...
trait CutOffReceivableVesa {
    ...
    private static function vesaApproval($array = [], $merge_into_group = true)
    {
        ...
        if ($merge_into_group && $list_cut_off->count() > 5) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/receivable/vesa-approval'),
                'deadline' => $list_cut_off->orderBy('form_date')->first()->form_date,
                'message' => 'please approve cut off receivable',
                'permission_slug' => 'approval.point.accounting.cut.off.receivable'
            ]);
        ...
        foreach ($list_cut_off->get() as $memo_journal) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/receivable/' . $memo_journal->id),
                'deadline' => $memo_journal->required_date ? : $memo_journal->formulir->form_date,
                'message' => 'please approve this cut off receivable ' . $memo_journal->formulir->form_number,
                'permission_slug' => 'approval.point.accounting.cut.off.receivable'
            ]);
        }
    ...
    private static function vesaReject($array = [], $merge_into_group = true)
    {
        ...
        if ($merge_into_group && $list_cut_off->count() > 5) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/receivable/vesa-rejected'),
                'deadline' => $list_cut_off->orderBy('form_date')->first()->form_date,
                'message' => 'Rejected, please edit your form',
                'permission_slug' => 'update.point.accounting.cut.off.receivable'
            ]);
        ...
        foreach ($list_cut_off->get() as $memo_journal) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/receivable/' . $memo_journal->id.'/edit'),
                'deadline' => $memo_journal->required_date ? : $memo_journal->formulir->form_date,
