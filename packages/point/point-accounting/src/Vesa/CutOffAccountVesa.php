...
trait CutOffAccountVesa {
    ...
    private static function vesaApproval($array = [], $merge_into_group = true)
    {
        ...
        if ($merge_into_group && $list_cut_off->count() > 5) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/account/vesa-approval'),
                'deadline' => $list_cut_off->orderBy('form_date')->first()->form_date,
                'message' => 'please approve cut off account',
                'permission_slug' => 'approval.point.accounting.cut.off.account'
            ]);
        ...
        foreach ($list_cut_off->get() as $memo_journal) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/account/' . $memo_journal->id),
    ...
    private static function vesaReject($array = [], $merge_into_group = true)
    {
        ...
        if ($merge_into_group && $list_cut_off->count() > 5) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/account/vesa-rejected'),
                'deadline' => $list_cut_off->orderBy('form_date')->first()->form_date,
                'message' => 'Rejected, please edit your form',
                'permission_slug' => 'update.point.accounting.cut.off.account'
            ]);
        ...
        foreach ($list_cut_off->get() as $memo_journal) {
            array_push($array, [
                'url' => url('accounting/point/cut-off/account/' . $memo_journal->id.'/edit'),
                'deadline' => $memo_journal->required_date ? : $memo_journal->formulir->form_date,
