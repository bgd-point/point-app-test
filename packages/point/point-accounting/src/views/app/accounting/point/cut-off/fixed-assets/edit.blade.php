...
<div id="page-content">
    ...
    <div class="panel panel-default"> 
        <div class="panel-body">
            <form action="{{url('accounting/point/cut-off/fixed-assets/'.$cut_off_fixed_assets->formulir_id)}}" method="post" class="form-horizontal form-bordered">
                {!! csrf_field() !!}
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <input type="text" name="form_date" id="form_date" class="form-control date input-datepicker" data-date-format="{{date_format_masking()}}" placeholder="{{date_format_masking()}}" value="{{date(date_format_get(), strtotime($cut_off_fixed_assets->formulir->form_date))}}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-3 control-label">Notes</label>
                    <div class="col-md-6">
                        <input name="notes" id="notes" class="form-control" value="{{ $cut_off_fixed_assets->formulir->notes }}" />
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        <select name="approval_to" class="selectize" style="width: 100%;" data-placeholder="Choose one..">
                            @foreach($list_user_approval as $user_approval)
                                @if($user_approval->may('approval.point.accounting.cut.off.fixed.assets'))
                ...
                <div class="form-group">
                    <div class="col-md-6 col-md-offset-3">
                        <button type="submit" class="btn btn-effect-ripple btn-primary">Submit</button>
                            ...
                            <a href="{{url('accounting/point/cut-off/fixed-assets/clear-tmp')}}" class="btn btn-effect-ripple btn-danger" data-toggle="modal">CLEAR</a>
...
function openDetail(coa_id, useful_life, index) {
    var coa = $("#coa-name-"+index).html();
    $("#modal-detail").modal();
    $("#index").val(index);
    $("#useful-life").val(useful_life);
    $("#modal-coa-id").val(coa_id);
    $("#modal-body-cutoff").html("loading..");
    $.ajax({
        url: '{{url("accounting/point/cut-off/fixed-assets/load-details")}}',
        type: 'get',
        data: {
            coa_id: coa_id,
        },
        success: function(data) {
            ...
            $("#modal-coa-name-cutoff-fixed-assets").html("<strong>"+coa+"</strong>");
            calculate();
            initFormatNumber();
        },
...
function updateTemp(index){
    $.ajax({
        url: '{{url("accounting/point/cut-off/fixed-assets/store-tmp-details")}}',
        type: 'post',
        data: {
