...
<div id="page-content">
    ...
    <form action="{{url('accounting/point/cut-off/account/send-request-approval')}}" method="post">
        {!! csrf_field() !!}
                        ...
                        <tbody>
                            @foreach($list_cut_off as $cut_off)
                             <tr id="list-{{$cut_off->formulir_id}}">
                                ...
                                <td><a href="{{ url('accounting/point/cut-off/account/'.$cut_off->id) }}">{{ $cut_off->formulir->form_number}}</a></td>
