...
<div id="page-content">
    ...
    <div class="panel panel-default"> 
        <div class="panel-body">
            <form action="{{url('accounting/point/cut-off/payable/'.$cut_off_payable->formulir_id)}}" method="post" class="form-horizontal form-bordered">
                {!! csrf_field() !!}
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <input type="text" name="form_date" id="form_date" class="form-control date input-datepicker" data-date-format="{{date_format_masking()}}" placeholder="{{date_format_masking()}}" value="{{date(date_format_get(), strtotime($cut_off_payable->formulir->form_date))}}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-3 control-label">Notes</label>
                    <div class="col-md-6">
                        <input name="notes" id="notes" class="form-control" value="{{ $cut_off_payable->formulir->notes }}" />
                    </div>
                </div>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody class="manipulate-row">
                                        ...
                                        <?php
                                            ...
                                            if (!$amount) {
                                                $payable_detail_coa = Point\PointAccounting\Models\CutOffPayableDetail::where('cut_off_payable_id', $cut_off_payable->id)->where('coa_id', $coa->id)->first();
                                                if ($payable_detail_coa) {
                                                    $amount = Point\PointAccounting\Models\CutOffPayableDetail::where('cut_off_payable_id', $cut_off_payable->id)->where('coa_id', $coa->id)->first()->amount;
                                                }
                                            }
                                        ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" id="row-amount-{{$index}}" onkeyup="reCalculate()" value="{{ $amount }}" name="amount[]" @if($coa->has_subledger) '' : readonly @endif class="form-control text-right format-quantity"></td>
                                        </tr>
                                        <?php $index++;?>
                                        @endforeach
                                    </tbody>
                    ...
                    <div class="col-md-6">
                        <select name="approval_to" class="selectize" style="width: 100%;" data-placeholder="Choose one..">
                            @foreach($list_user_approval as $user_approval)
                                @if($user_approval->may('approval.point.accounting.cut.off.payable'))
