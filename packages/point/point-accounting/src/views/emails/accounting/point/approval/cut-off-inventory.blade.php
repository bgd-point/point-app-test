...
<body>
    ...
    <div class="invoice-box">
        <p align="justify">
            Hi, you have an approval request for Cut Off Account Inventory from <strong>{{ $requester }}</strong>. <br>
            We would like to inform the details as follows :
        </p>
        <?php
            $list_coa = Point\Framework\Models\Master\Coa::active()->where('coa_category_id', 4)->get();
        ?>
        @foreach($list_data as $cut_off)
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                @foreach($list_coa as $coa)
                    <?php
                        $cut_off_inventory_detail = Point\PointAccounting\Models\CutOffInventoryDetail::where('coa_id', $coa->id)->where('cut_off_inventory_id', $cut_off->id)->get();
                        $amount = Point\PointAccounting\Models\CutOffInventoryDetail::where('coa_id', $coa->id)->where('cut_off_inventory_id', $cut_off->id)->sum('amount');
                        $total_amount += $amount;

                        if (!$cut_off_inventory_detail->count()) {
                            continue;
                        }
                    ?>
                    <tr class="heading">
                        ...
                        <td align="right">Stock In Database</td>
                        <td align="right">Stock</td>
                        <td align="right">Amount</td>
                    </tr>
                    @foreach($cut_off_inventory_detail as $account)
                    <tr class="item">
                        <td align="left">{{ $account->item->codeName }}</td>
                        <td align="left">{{ $account->warehouse->name }}</td>
                        <td align="left">{{ $account->notes }}</td>
                        <td align="right">{{ number_format_quantity($account->stock_in_database) }}</td>
                        <td align="right">{{ number_format_quantity($account->stock) }}</td>
                ...
                <tr>
                    <td colspan="3">
                       ...
                       <a href="{{ $url . '/accounting/point/cut-off/inventory/'.$cut_off->id.'/approve?token='.$token }}"> 
                            <input type="button" class="btn btn-success" value="APPROVE">
                       </a>
                       <a href="{{ $url . '/accounting/point/cut-off/inventory/'.$cut_off->id.'/reject?token='.$token }}">
