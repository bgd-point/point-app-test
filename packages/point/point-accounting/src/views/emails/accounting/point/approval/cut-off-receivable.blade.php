...
<body>
    ...
    <div class="invoice-box">
        <p align="justify">
            Hi, you have an approval request for Cut Off Account Receivable from <strong>{{ $requester }}</strong>. <br>
            We would like to inform the details as follows :
        </p>
        <?php
            $list_coa = Point\Framework\Models\Master\Coa::active()->where('coa_category_id', 3)->get();
        ?>
        @foreach($list_data as $cut_off)
            <table cellpadding="0" cellspacing="0" style="padding: 20px 0;">
                <tr>
                    ...
                    <td>:</td>
                    <td>
                        <a href="{{ url('accounting/point/cut-off/receivable/'.$cut_off->id) }}">
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                @foreach($list_coa as $coa)
                    <?php
                        $cut_off_receivable_detail = Point\PointAccounting\Models\CutOffReceivableDetail::where('coa_id', $coa->id)->where('cut_off_receivable_id', $cut_off->id)->get();
                        $amount = Point\PointAccounting\Models\CutOffReceivableDetail::where('coa_id', $coa->id)->where('cut_off_receivable_id', $cut_off->id)->sum('amount');
                        $total_amount += $amount;
                ...
                <tr>
                    <td colspan="3">
                       ...
                       <a href="{{ $url . '/accounting/point/cut-off/receivable/'.$cut_off->id.'/approve?token='.$token }}"> 
                            <input type="button" class="btn btn-success" value="APPROVE">
                       </a>
                       <a href="{{ $url . '/accounting/point/cut-off/receivable/'.$cut_off->id.'/reject?token='.$token }}">
