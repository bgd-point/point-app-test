...
class CutOffHelper
{
    public static function journal($cut_off)
    {
        $cut_off_account = CutOffAccount::joinFormulir()
            ...
            ->approvalApproved()
            ->open()
            ->where('form_date', '=', $cut_off->formulir->approval_at)
    ...
    private static function checkFixedAssets($cut_off_account)
    {
        foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
            if ($cut_off_account_detail->coa->subledger_type == get_class(new FixedAsset())) {
                $amount = CutOffFixedAssets::getSubledgerAmount($cut_off_account->formulir->approval_at, $cut_off_account_detail->coa_id);
    ...
    private static function checkInventory($cut_off_account)
    {
        foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
            if ($cut_off_account_detail->coa->subledger_type == get_class(new Item())) {
                $amount = CutOffInventory::getSubledgerAmount($cut_off_account->formulir->approval_at, $cut_off_account_detail->coa_id);
                $position = JournalHelper::position($cut_off_account_detail->coa_id);
    ...
    private static function checkAccountPerson($cut_off_account)
    {
        foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
            if ($cut_off_account_detail->coa->subledger_type == get_class(new Person())) {
                $account_payable_amount = CutOffPayable::getSubledgerAmount($cut_off_account->formulir->approval_at, $cut_off_account_detail->coa_id);
                $account_receivable_amount = CutOffReceivable::getSubledgerAmount($cut_off_account->formulir->approval_at, $cut_off_account_detail->coa_id);
                $position = JournalHelper::position($cut_off_account_detail->coa_id);
    ...
    {
        // INVENTORY
        $inventories = Inventory::where('form_date', '<=', $cut_off_account->formulir->approval_at)
            ->select('inventory.*')
            ->groupBy('item_id')
        ...
        foreach ($list_co_journal as $co_journal) {
            $journal_value = JournalHelper::getTotalValue($co_journal->coa_id, $cut_off_account->formulir->approval_at);
            ...
            foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
                ...
                if ($coa) {
                    ...
                    foreach ($list_fixed_assets_detail as $fixed_assets_detail) {
                        $journal_value = JournalHelper::getTotalValue($cut_off_account_detail->coa_id, $cut_off_account->formulir->approval_at);
                        $position = JournalHelper::position($cut_off_account_detail->coa_id);
                        $total = abs($journal_value->debit - $journal_value->credit);
                        if ($total > 0) {
                            $journal = new Journal();
    ...
    private static function accountInventory($cut_off_account, $cut_off_account_detail)
    {
        ...
        foreach ($cut_off_inventory->cutOffInventoryDetail as $cut_off_inventory_detail) {
            if ($cut_off_inventory_detail->stock > 0) {
                ...
                $inventory->form_date = date('Y-m-d H:i:s', strtotime($cut_off_inventory->formulir->approval_at));
...
//        }
    ...
    public static function cancel($formulir_id)
    {
        ...
        $formulir_relation = Formulir::where('form_date', $formulir->approval_at)
