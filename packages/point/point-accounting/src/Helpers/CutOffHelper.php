...
class CutOffHelper
{
    public static function journal($cut_off)
    {
        $cut_off_account = CutOffAccount::joinFormulir()
            ...
            ->approvalApproved()
            ->open()
            ->where('form_date', '=', $cut_off->formulir->created_at)
...
//            return false;
    ...
    private static function checkFixedAssets($cut_off_account)
    {
        foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
            if ($cut_off_account_detail->coa->subledger_type == get_class(new FixedAsset())) {
                $amount = CutOffFixedAssets::getSubledgerAmount($cut_off_account->formulir->created_at, $cut_off_account_detail->coa_id);
    ...
    private static function checkInventory($cut_off_account)
    {
        foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
            if ($cut_off_account_detail->coa->subledger_type == get_class(new Item())) {
                $amount = CutOffInventory::getSubledgerAmount($cut_off_account->formulir->created_at, $cut_off_account_detail->coa_id);
                $position = JournalHelper::position($cut_off_account_detail->coa_id);
    ...
    private static function checkAccountPerson($cut_off_account)
    {
        foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
            if ($cut_off_account_detail->coa->subledger_type == get_class(new Person())) {
                $account_payable_amount = CutOffPayable::getSubledgerAmount($cut_off_account->formulir->created_at, $cut_off_account_detail->coa_id);
                $account_receivable_amount = CutOffReceivable::getSubledgerAmount($cut_off_account->formulir->created_at, $cut_off_account_detail->coa_id);
                $position = JournalHelper::position($cut_off_account_detail->coa_id);
    ...
    {
        // INVENTORY
        $inventories = Inventory::where('form_date', '<=', $cut_off_account->formulir->created_at)
            ->select('inventory.*')
            ->groupBy('item_id')
        ...
        if ($inventories) {
            foreach($inventories as $inventory) {
                ...
                $emptying_inventory->form_date = $cut_off_account->formulir->created_at;
                ...
                $journal = new Journal();
                $journal->form_date = date('Y-m-d H:i:s', strtotime($cut_off_account->formulir->created_at));
        ...
        foreach ($list_account_payable_receivable as $account_payable_receivable) {
            ...
            if ($total_debt > 0 && $account_payable_receivable < $cut_off_account->formulir->created_at ) {
        ...
        foreach ($list_co_journal as $co_journal) {
            $journal_value = JournalHelper::getTotalValue($co_journal->coa_id, $cut_off_account->formulir->created_at);
        ...
        $cut_off_fixed_assets = CutOffFixedAssets::joinFormulir()
            ...
            ->open()
            ->notArchived()
            ->where('form_date', 'like', date('Y-m-d', strtotime($cut_off_account->formulir->created_at)) . '%')
            ->selectOriginal()
            ->orderby('id', 'desc')
            ->first();
            ...
            foreach ($cut_off_account->cutOffAccountDetail as $cut_off_account_detail) {
                ...
                if ($coa) {
                    ...
                    foreach ($list_fixed_assets_detail as $fixed_assets_detail) {
                        $journal_value = JournalHelper::getTotalValue($cut_off_account_detail->coa_id, $cut_off_account->formulir->created_at);
                        $position = JournalHelper::position($cut_off_account_detail->coa_id);
                        $total = abs($journal_value->debit - $journal_value->credit);
                        if ($total > 0) {
                            $journal = new Journal();
    ...
    private static function accountInventory($cut_off_account, $cut_off_account_detail)
    {
        ...
        foreach ($cut_off_inventory->cutOffInventoryDetail as $cut_off_inventory_detail) {
            if ($cut_off_inventory_detail->stock > 0) {
                ...
                $inventory->form_date = date('Y-m-d H:i:s', strtotime($cut_off_inventory->formulir->created_at));
...
//        }
    ...
    public static function cancel($formulir_id)
    {
        ...
        $formulir_relation = Formulir::where('form_date', $formulir->created_at)
