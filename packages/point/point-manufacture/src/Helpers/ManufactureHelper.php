...
class ManufactureHelper
{
    ...
    public static function createInput($request, $formulir)
    {
        ...
        for ($i = 0; $i < count($material_id); $i++) {
            ...
            $raw_material->unit = $unit->name;
            $raw_material->converter = number_format_db($unit->converter);
            $raw_material->cogs = InventoryHelper::getCostOfSales($input->formulir->approval_at, $raw_material->material_id, $raw_material->warehouse_id) * $raw_material->quantity;
            $raw_material->save();
        }
    ...
    public static function approveInput($input_approval)
    {
        foreach ($input_approval->material as $material) {
            ...
            $inventory->cogs = InventoryHelper::getCostOfSales($input_approval->formulir->approval_at, $material->material_id, $material->warehouse_id);
            $inventory->price = $inventory->cogs;
            $inventory->form_date = $input_approval->formulir->approval_at;
