            ...
            <div class="tab-content">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-vcenter">
                                            ...
                                            @foreach(\Point\Framework\Helpers\ReferHelper::getRefers(get_class($payment_order), $payment_order->id) as $refer)
                                                <?php
                                                $payment_order_detail = \Point\PointPurchasing\Models\Inventory\Basic\PaymentOrderDetail::find($refer->to_id);
                ...
                <div class="tab-pane" id="block-tabs-settings">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                @if(formulir_view_edit($payment_order->formulir, 'update.point.purchasing.basic.payment.order'))
                                    <a href="{{url('purchasing/point/payment-order/basic/'.$payment_order->id.'/edit')}}"
                                       class="btn btn-effect-ripple btn-info"><i class="fa fa-pencil"></i> Edit</a>
                                @endif
                                @if(formulir_view_cancel($payment_order->formulir, 'delete.point.purchasing.basic.payment.order'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureCancelForm('{{url('formulir/cancel')}}',
                                               '{{ $payment_order->formulir_id }}',
                                               'delete.point.purchasing.basic.payment.order')"><i class="fa fa-times"></i>
                                        Cancel Form</a>
                                @endif
                                @if(formulir_view_close($payment_order->formulir, 'update.point.purchasing.basic.payment.order'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureCloseForm({{$payment_order->formulir_id}},'{{url('formulir/close')}}')">Close
                                        Form</a>
                                @endif
                                @if(formulir_view_reopen($payment_order->formulir, 'update.point.purchasing.basic.payment.order'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureReopenForm({{$payment_order->formulir_id}},'{{url('formulir/reopen')}}')">Reopen
                                        Form</a>
                                @endif
                                @if(formulir_view_email_vendor($payment_order->formulir, 'create.point.purchasing.basic.payment.order'))
                                    <form action="{{url('purchasing/point/payment-order/basic/send-email-payment')}}" method="post">
                                        {!! csrf_field() !!}
                    ...
                    @if(formulir_view_approval($payment_order->formulir, 'approval.point.purchasing.basic.payment.order'))
                        <fieldset>
                            <div class="form-group">
                                <div class="col-md-12">
                            ...
                            <div class="form-group">
                                <div class="col-md-6">
                                    <form action="{{url('purchasing/point/payment-order/basic/'.$payment_order->id.'/approve')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                ...
                                <div class="col-md-6">
                                    <form action="{{url('purchasing/point/payment-order/basic/'.$payment_order->id.'/reject')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                            ...
                                            @foreach($list_payment_order_archived as $payment_order_archived)
                                                <tr>
                                                    <td class="text-center">
                                                        <a href="{{ url('purchasing/point/payment-order/basic/'.$payment_order_archived->id.'/archived') }}"
