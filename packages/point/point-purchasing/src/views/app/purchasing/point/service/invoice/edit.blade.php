    ...
    <div id="page-content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="{{url('purchasing/point/service/invoice/'.$invoice->id)}}" method="post"
                                ...
                                <table id="service-datatable" class="table table-striped">
                                    ...
                                    @foreach($invoice->services as $invoice_service)
                                        <tr>
                                            ...
                                            <td>
                                                <select id="service-allocation-id-{{$service_counter}}" name="service_allocation_id[]" class="selectize" style="width: 100%;" data-placeholder="Choose one.." onchange="selectService(this.value, {{$service_counter}})">
                                                    @foreach($list_allocation as $allocation)
                                                    <option value="{{$allocation->id}}" @if($allocation->id == $invoice_service->allocation_id) selected @endif >{{$allocation->name}}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
                                ...
                                <table id="item-datatable" class="table table-striped">
                                    ...
                                    @foreach($invoice->items as $invoice_item)
                                        <tr>
                                            ...
                                            <td>
                                                <select id="allocation-id-{{$counter}}" name="allocation_id[]" class="selectize" style="width: 100%;" data-placeholder="Choose one.." onchange="selectService(this.value, {{$service_counter}})">
                                                    @foreach($list_allocation as $allocation)
                                                    <option value="{{$allocation->id}}" @if($allocation->id == $invoice_item->allocation_id) selected @endif >{{$allocation->name}}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td>
