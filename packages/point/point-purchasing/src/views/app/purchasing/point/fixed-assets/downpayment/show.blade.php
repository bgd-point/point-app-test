...
@section('content')
    <div id="page-content">
        <ul class="breadcrumb breadcrumb-top">
            ...
            <li><a href="{{ url('purchasing/point/fixed-assets/downpayment') }}">Downpayment</a></li>
            <li>Show</li>
        </ul>
                                ...
                                <div class="form-group">
                                    <div class="col-md-12">
                                        <legend><i class="fa fa-angle-right"></i> REF#
                                            @if($downpayment->purchasing_order_id !== null && $downpayment->purchasing_order_id > 0)
                                                <a target="_blank" href="{{ url('purchasing/point/fixed-assets/purchase-order/'.$downpayment->purchasing_order_id) }}">{{$downpayment->purchaseOrder->formulir->form_number}}</a>@endif
                                        </legend>
                                    </div>
                                </div>
                ...
                <div class="tab-pane" id="block-tabs-settings">
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                ...
                                @elseif(formulir_view_cancel_or_request_cancel($downpayment->formulir, 'delete.point.purchasing.downpayment.fixed.assets', 'approval.point.purchasing.downpayment.fixed.assets') == 2)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureRequestCancelForm('{{url('formulir/requestCancel')}}', '{{ $downpayment->formulir_id }}', 'delete.point.purchasing.downpayment.fixed.assets')">
                                        <i class="fa fa-times"></i> 
                                        Request Cancel Form
                                    </a>
                                @endif
                    ...
                    @if(formulir_view_approval($downpayment->formulir, 'approval.point.purchasing.downpayment.fixed.assets'))
                        <fieldset>
                            ...
                            <div class="form-group">
                                <div class="col-md-6">
                                    <form action="{{url('purchasing/point/fixed-assets/downpayment/'.$downpayment->id.'/approve')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                ...
                                <div class="col-md-6">
                                    <form action="{{url('purchasing/point/fixed-assets/downpayment/'.$downpayment->id.'/reject')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                        <div class="input-group">
                                            <input type="text" name="approval_message" class="form-control"
                                            ...
                                            @foreach($list_downpayment_archived as $downpayment_archived)
                                                <tr>
                                                    <td class="text-center">
                                                        <a href="{{ url('purchasing/point/fixed-assets/downpayment/'.$downpayment_archived->formulirable_id.'/archived') }}"
                                                           data-toggle="tooltip"
                                                           title="Show"
