            ...
            <div class="tab-content">
                ...
                <div class="tab-pane" id="block-tabs-settings">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                @if(formulir_view_edit($invoice->formulir, 'update.point.purchasing.service.invoice'))
                                ...
                                @if(formulir_view_cancel_or_request_cancel($purchase_requisition->formulir, 'delete.point.purchasing.requisition', 'approval.point.purchasing.requisition') == 1)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureCancelForm('{{url('formulir/cancel')}}', '{{ $purchase_requisition->formulir_id }}','approval.point.purchasing.requisition')">
                                        <i class="fa fa-times"></i> 
                                        Cancel Form
                                    </a>
                                @elseif(formulir_view_cancel_or_request_cancel($purchase_requisition->formulir, 'delete.point.purchasing.requisition', 'approval.point.purchasing.requisition') == 2)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureRequestCancelForm('{{url('formulir/requestCancel')}}', '{{ $purchase_requisition->formulir_id }}', 'delete.point.purchasing.requisition')">
                                        <i class="fa fa-times"></i> 
                                        Request Cancel Form
                                    </a>
                                @endif
                                ...
                                @if(formulir_view_close($invoice->formulir, 'update.point.purchasing.service.invoice'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureCloseForm({{$invoice->formulir_id}},'{{url('formulir/close')}}')">Close
                                        Form</a>
                                @endif
                                @if(formulir_view_reopen($invoice->formulir, 'update.point.purchasing.service.invoice'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureReopenForm({{$invoice->formulir_id}},'{{url('formulir/reopen')}}')">Reopen
                                        Form</a>
                                @endif
                                @if(formulir_view_email_vendor($invoice->formulir, 'create.point.purchasing.service.invoice'))
                                    <form action="{{url('purchasing/point/service/invoice/send-email')}}" method="post">
                                        {!! csrf_field() !!}
                                    ...
                                    href="{{url('purchasing/point/service/invoice/'.$invoice->id.'/export')}}">Export PDF</a>
                                @endif
                            </div>
                        </div>
                    </fieldset>

                    @if(formulir_view_approval($invoice->formulir, 'approval.point.purchasing.service.invoice'))
                        <fieldset>
                            <div class="form-group">
                                <div class="col-md-12">
                            ...
                            <div class="form-group">
                                <div class="col-md-6">
                                    <form action="{{url('purchasing/point/service/invoice/'.$invoice->id.'/approve')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                ...
                                <div class="col-md-6">
                                    <form action="{{url('purchasing/point/service/invoice/'.$invoice->id.'/reject')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                            ...
                                            @foreach($list_invoice_archived as $invoice_archived)
                                                <tr>
                                                    <td class="text-center">
                                                        <a href="{{ url('purchasing/point/service/invoice/'.$invoice_archived->id.'/archived') }}"
