            ...
            <div class="tab-content">
                ...
                <div class="tab-pane" id="block-tabs-settings">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                @if(formulir_view_edit($invoice->formulir, 'update.point.purchasing.basic.invoice'))
                                ...
                                @endif
                                @if(formulir_view_cancel($invoice->formulir, 'delete.point.purchasing.basic.invoice'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureCancelForm('{{url('formulir/cancel')}}',
                                               '{{ $invoice->formulir_id }}',
                                               'delete.point.purchasing.basic.invoice')"><i class="fa fa-times"></i> Cancel
                                        Form</a>
                                @endif
                                @if(formulir_view_close($invoice->formulir, 'update.point.purchasing.basic.invoice'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureCloseForm({{$invoice->formulir_id}},'{{url('formulir/close')}}')">Close
                                        Form</a>
                                @endif
                                @if(formulir_view_reopen($invoice->formulir, 'update.point.purchasing.basic.invoice'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureReopenForm({{$invoice->formulir_id}},'{{url('formulir/reopen')}}')">Reopen
                                        Form</a>
                                @endif
                                @if(formulir_view_email_vendor($invoice->formulir, 'create.point.purchasing.basic.invoice'))
                                            ...
                                            @foreach($list_invoice_archived as $invoice_archived)
                                                <tr>
                                                    <td class="text-center">
                                                        <a href="{{ url('purchasing/point/invoice/basic/'.$invoice_archived->id.'/archived') }}"
