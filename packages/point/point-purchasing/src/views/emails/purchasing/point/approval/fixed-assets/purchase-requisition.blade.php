...
<body>
    <div class="invoice-box">
        ...
        @foreach($list_data as $purchase_requisition)
            <?php $purchase_requisition = Point\PointPurchasing\Models\FixedAssets\FixedAssetsPurchaseRequisition::find($purchase_requisition['id']); ?>
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                @foreach($purchase_requisition->details as $detail)
                    <tr class="item">
                        ...
                        <td>{{ $detail->name }}</td>
                        <td>{{ number_format_quantity($detail->quantity) }} Unit</td>
                        <td>{{ number_format_quantity($detail->price) }}</td>
                        <td>{{ $detail->allocation->name }}</td>
                    </tr>
                @endforeach
                <tr></tr>
                <tr>
                    <td colspan="6" >
                        ...
                        <a href="{{ $url . '/purchasing/point/fixed-assets/purchase-requisition/'.$purchase_requisition->id.'/approve?token='.$token }}"><input
                                    type="button" class="btn btn-success" value="Approve"></a>
                        <a href="{{ $url . '/purchasing/point/fixed-assets/purchase-requisition/'.$purchase_requisition->id.'/reject?token='.$token }}"><input
        ...
        <br>
        <a href="{{ $url . '/purchasing/point/fixed-assets/purchase-requisition/approve-all/?formulir_id='.$array_formulir_id.'&token='.$token }}">
            <input type="button" class="btn btn-primary" value="Approve All">
        </a>
        <a href="{{ $url . '/purchasing/point/fixed-assets/purchase-requisition/reject-all/?formulir_id='.$array_formulir_id.'&token='.$token }}">
