...
<header class="clearfix">
    ...
    <div id="" style="text-align: right;margin-left: 50px;">
        <div><b>{{ strtoupper($purchase_order->person->name) }}</b></div>
        <div>{{ strtoupper($purchase_order->person->address) }}</div>
        <div>{{ $purchase_order->person->phone }}</div>
    </div>
</header>
<main>
    <table>
        ...
        @foreach($purchase_order->services as $purchase_order_service)
            <tr>
                <td class="text-left desc">{{ $purchase_order_service->service->name }} {{ $purchase_order_service->service_notes }}</td>
                <td class="text-right">{{ number_format_price($purchase_order_service->price, 0) }}</td>
                <td class="text-right">{{ number_format_price($purchase_order_service->quantity, 0) }} {{ $purchase_order_service->unit }}</td>
                <td class="text-right">{{ number_format_price($purchase_order_service->discount, 0) }}</td>
                <td class="text-right">{{ number_format_price(($purchase_order_service->quantity * $purchase_order_service->price) - ($purchase_order_service->quantity * $purchase_order_service->price * $purchase_order_service->discount / 100), 0) }}</td>
            </tr>
        @endforeach
        </tbody>
        <tfoot>
