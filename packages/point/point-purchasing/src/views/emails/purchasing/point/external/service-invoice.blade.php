...
<body>
    <div class="invoice-box">
        Hello Mr/Mrs/Ms/ {{ $invoice->person->name }},<br/>You have an email invoice from <br>
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            @foreach($invoice->services as $invoice_service)
                <tr class="item">
                    <td>{{ $invoice_service->service->name }}</td>
                    <td>{{ number_format_quantity($invoice_service->quantity) }}</td>
                    <td>{{ $invoice_service->unit}}</td>
                    <td align="right">{{ number_format_price($invoice_service->price) }}</td>
                    <td align="right">{{ number_format_price($invoice_service->discount) }}</td>
                    <td align="right">
                        {{ number_format_price($invoice_service->quantity * $invoice_service->price - ($invoice_service->quantity * $invoice_service->price * $invoice_service->discount/100)) }}
                    </td>
                </tr>
            @endforeach
            ...
            @foreach($invoice->items as $invoice_item)
                <tr>
                    ...
                    <td align="right">{{ number_format_quantity($invoice_item->quantity) }} {{ $invoice_item->unit }}</td>
                    <td align="right">{{ number_format_quantity($invoice_item->price) }}</td>
                    <td align="right">{{ number_format_quantity($invoice_item->discount) }}</td>
                    <td align="right">{{ number_format_quantity(($invoice_item->quantity * $invoice_item->price) - ($invoice_item->quantity * $invoice_item->price * $invoice_item->discount / 100)) }}</td>
                </tr>
            @endforeach
            @endif
            <tr>
