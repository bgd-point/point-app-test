...
class PurchaseOrderHelper
{
    ...
    public static function create(Request $request, $formulir)
    {
        ...
        for ($i=0 ; $i < count($request->input('item_id')) ; $i++) {
            ...
            $purchase_order_item->point_purchasing_order_id = $purchase_order->id;
            $purchase_order_item->item_id = $request->input('item_id')[$i];
            $purchase_order_item->allocation_id = $request->input('allocation_id')[$i];
            $purchase_order_item->quantity = number_format_db($request->input('item_quantity')[$i]);
            $purchase_order_item->price = number_format_db($request->input('item_price')[$i]);
            $purchase_order_item->discount = number_format_db($request->input('item_discount')[$i]);
            $purchase_order_item->unit = $request->input('item_unit_name')[$i];
            $purchase_order_item->converter = 1;
