...
class FixedAssetsPurchaseOrderApprovalController extends Controller
{
    ...
    public function requestApproval()
    {
        ...
        $view = view('point-purchasing::app.purchasing.point.fixed-assets.purchase-order.request-approval');
        $view->list_purchase_order = FixedAssetsPurchaseOrder::selectRequestApproval()->paginate(100);
        return $view;
    }
    ...
    {
        access_is_allowed('create.point.purchasing.order.fixed.assets');
        $list_approver = FixedAssetsPurchaseOrder::selectApproverList(app('request')->input('formulir_id'));
        $request = $request->input();
        $token = md5(date('ymdhis'));

        foreach ($list_approver as $data_approver) {
            $list_purchase_order = FixedAssetsPurchaseOrder::selectApproverRequest(app('request')->input('formulir_id'), $data_approver->approval_to);
            $array_formulir_id = [];
            ...
            \Queue::push(function ($job) use ($approver, $data, $request) {
                ...
                \Mail::send('point-purchasing::emails.purchasing.point.approval.fixed-assets.purchase-order', $data, function ($message) use ($approver) {
    ...
    public function approve(Request $request, $id)
    {
        ...
        FormulirHelper::approve($purchase_order->formulir, $approval_message, 'approval.point.purchasing.order.fixed.assets', $token);
        FixedAssetsPurchaseOrderHelper::registerToExpedition($purchase_order);
    ...
    public function reject(Request $request, $id)
    {
        ...
        FormulirHelper::reject($purchase_order->formulir, $approval_message, 'approval.point.purchasing.order.fixed.assets', $token);
    ...
    public function approveAll()
    {
        ...
        foreach ($array_formulir_id as $id) {
            $purchase_order = FixedAssetsPurchaseOrder::where('formulir_id', $id)->first();
            FormulirHelper::approve($purchase_order->formulir, $approval_message, 'approval.point.purchasing.order.fixed.assets', $token);
