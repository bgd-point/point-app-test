...
class FixedAssetsPurchaseRequisitionApprovalController extends Controller
{
    ...
    public function requestApproval()
    {
        ...
        $view = view('point-purchasing::app.purchasing.point.fixed-assets.purchase-requisition.request-approval');
        $view->list_purchase_requisition = FixedAssetsPurchaseRequisition::selectRequestApproval()->paginate(100);
        return $view;
    }
    ...
    {
        access_is_allowed('create.point.purchasing.requisition.fixed.assets');
        $list_approver = FixedAssetsPurchaseRequisition::selectApproverList(app('request')->input('formulir_id'));
        $request = $request->input();
        $token = md5(date('ymdhis'));

        foreach ($list_approver as $data_approver) {
            $list_purchase_requisition = FixedAssetsPurchaseRequisition::selectApproverRequest(app('request')->input('formulir_id'), $data_approver->approval_to);
            $array_formulir_id = [];
            ...
            \Queue::push(function ($job) use ($approver, $data, $request) {
                ...
                \Mail::send('point-purchasing::emails.purchasing.point.approval.fixed-assets.purchase-requisition', $data, function ($message) use ($approver) {
                    $message->to($approver->email)->subject('request approval purchase requisition fixed assets #' . date('ymdHi'));
                });
                $job->delete();
            });
    ...
    public function approve(Request $request, $id)
    {
        ...
        FormulirHelper::approve($purchase_requisition->formulir, $approval_message, 'approval.point.purchasing.requisition.fixed.assets', $token);
    ...
    public function reject(Request $request, $id)
    {
        ...
        FormulirHelper::reject($purchase_requisition->formulir, $approval_message, 'approval.point.purchasing.requisition.fixed.assets', $token);
        timeline_publish('reject', 'purchase requisition fixed assets ' . $purchase_requisition->formulir->form_number . ' rejected', $this->getUserForTimeline($request, $purchase_requisition->formulir->approval_to));
    ...
    public function approveAll()
    {
        ...
        foreach ($array_formulir_id as $id) {
            $purchase_requisition = FixedAssetsPurchaseRequisition::where('formulir_id', $id)->first();
            FormulirHelper::approve($purchase_requisition->formulir, $approval_message, 'approval.point.purchasing.requisition.fixed.assets', $token);
