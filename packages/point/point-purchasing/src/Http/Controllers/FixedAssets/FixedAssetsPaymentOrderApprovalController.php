...
class FixedAssetsPaymentOrderApprovalController extends Controller
{
    use ValidationTrait, RequestApprovalTrait;

    public function requestApproval()
    {
        access_is_allowed('create.point.purchasing.payment.order.fixed.assets');
        
        $view = view('point-purchasing::app.purchasing.point.fixed-assets.payment-order.request-approval');
        $view->list_payment_order = FixedAssetsPaymentOrder::selectRequestApproval()->paginate(100);
        return $view;
    }
    ...
    public function sendRequestApproval(Request $request)
    {
        ...
        $list_approver = FixedAssetsPaymentOrder::selectApproverList(app('request')->input('formulir_id'));
        $request = $request->input();
        $token = md5(date('ymdhis'));

        foreach ($list_approver as $data_approver) {
            $list_payment_order = FixedAssetsPaymentOrder::selectApproverRequest(app('request')->input('formulir_id'), $data_approver->approval_to);
            $array_formulir_id = [];
    ...
    public function approve(Request $request, $id)
    {
        ...
        FormulirHelper::approve($payment_order->formulir, $approval_message, 'approval.point.purchasing.payment.order.fixed.assets', $token);
    ...
    public function reject(Request $request, $id)
    {
        ...
        FormulirHelper::reject($payment_order->formulir, $approval_message, 'approval.point.purchasing.payment.order.fixed.assets', $token);
    ...
    public function approveAll()
    {
        ...
        foreach ($array_formulir_id as $id) {
            $payment_order = FixedAssetsPaymentOrder::where('formulir_id', $id)->first();
            self::addPaymentReference($payment_order);
            FormulirHelper::approve($payment_order->formulir, $approval_message, 'approval.point.purchasing.payment.order.fixed.assets', $token);
