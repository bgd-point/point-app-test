    ...
    <div id="page-content">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                <form action="{{ url('expedition/point/invoice') }}" method="get" class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <input type="text" name="search" id="search" class="form-control"
                                   placeholder="Search Form Number / Expedition..." value="{{\Input::get('search')}}"
                        ...
                        <div class="col-sm-12">
                            ...
                            @if(auth()->user()->may('read.point.expedition.invoice'))
                                <a class="btn btn-effect-ripple btn-effect-ripple btn-info button-export" id="btn-pdf" href="{{url('expedition/point/invoice/pdf?date_from='.\Input::get('date_from').'&date_to='.\Input::get('date_to').'&search='.\Input::get('search').'&order_by='.\Input::get('order_by').'&order_type='.\Input::get('order_type').'&status='.\Input::get('status'))}}"> export to PDF</a>
                            @endif
                        </div>
                    </div>
                </form>
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($list_invoice as $invoice)
                            <tr id="list-{{$invoice->formulir_id}}">
                                ...
                                <td>{!! get_url_person($invoice->expedition_id) !!}</td>
...
function selectData(order_by, order_type) {
    ...
    var url = '{{url()}}/expedition/point/invoice/?order_by='+order_by+'&order_type='+order_type+'&status='+status+'&date_from='+date_from+'&date_to='+date_to+'&search='+search;
    location.href = url;
}
</script>
@stop