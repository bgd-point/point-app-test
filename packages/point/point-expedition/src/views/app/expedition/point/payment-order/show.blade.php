            ...
            <div class="tab-content">
                <div class="tab-pane active" id="block-tabs-home">
                    <div class="form-horizontal form-bordered">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6 content-show">
                                {!! get_url_person($payment_order->expedition_id) !!}
                            </div>
                        </div>
                        <div class="form-group">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped">
                                            ...
                                            @foreach(\Point\Framework\Helpers\ReferHelper::getRefers(get_class($payment_order), $payment_order->id) as $refer)
                                                <?php
                                                $payment_order_detail = \Point\PointExpedition\Models\PaymentOrderDetail::find($refer->to_id);
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right"><h4><b>{{number_format_quantity($payment_order->total_payment)}}</b></h4></td>
                                            </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                ...
                <div class="tab-pane" id="block-tabs-settings">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <div class="col-md-12">
                                @if(formulir_view_edit($payment_order->formulir, 'update.point.expedition.payment.order'))
                                    <a href="{{url('expedition/point/payment-order/'.$payment_order->id.'/edit')}}"
                                       class="btn btn-effect-ripple btn-info"><i class="fa fa-pencil"></i> Edit</a>
                                @endif
                                @if(formulir_view_cancel_or_request_cancel($payment_order->formulir, 'delete.point.expedition.payment.order', 'approval.point.expedition.payment.order') == 1)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureCancelForm('{{url('formulir/cancel')}}', '{{ $payment_order->formulir_id }}','approval.point.expedition.payment.order')">
                                        <i class="fa fa-times"></i> 
                                        Cancel Form
                                    </a>
                                @elseif(formulir_view_cancel_or_request_cancel($payment_order->formulir, 'delete.point.expedition.payment.order', 'approval.point.expedition.payment.order') == 2)
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger" onclick="secureRequestCancelForm(this, '{{url('formulir/requestCancel')}}', '{{ $payment_order->formulir_id }}', 'delete.point.expedition.payment.order')">
                                        <i class="fa fa-times"></i> 
                                        Request Cancel Form
                                    </a>
                                @endif
                                @if(formulir_view_close($payment_order->formulir, 'update.point.expedition.payment.order'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureCloseForm({{$payment_order->formulir_id}},'{{url('formulir/close')}}')">Close
                                        Form</a>
                                @endif
                                @if(formulir_view_reopen($payment_order->formulir, 'update.point.expedition.payment.order'))
                                    <a href="javascript:void(0)" class="btn btn-effect-ripple btn-danger"
                                       onclick="secureReopenForm({{$payment_order->formulir_id}},'{{url('formulir/reopen')}}')">Reopen
                                        Form</a>
                                @endif
                                @if(formulir_view_email_vendor($payment_order->formulir, 'create.point.expedition.payment.order'))
                                    <form action="{{url('expedition/point/payment-order/send-email-payment')}}" method="post">
                                        {!! csrf_field() !!}
                    ...
                    @if(formulir_view_approval($payment_order->formulir, 'approval.point.expedition.payment.order'))
                        <fieldset>
                            <div class="form-group">
                                <div class="col-md-12">
                            ...
                            <div class="form-group">
                                <div class="col-md-6">
                                    <form action="{{url('expedition/point/payment-order/'.$payment_order->id.'/approve')}}" method="post">
                                        {!! csrf_field() !!}
                                ...
                                <div class="col-md-6">
                                    <form action="{{url('expedition/point/payment-order/'.$payment_order->id.'/reject')}}"
                                          method="post">
                                        {!! csrf_field() !!}
                                        <div class="input-group">
                                            <input type="text" name="approval_message" class="form-control" placeholder="Message">
                                            ...
                                            @foreach($list_payment_order_archived as $payment_order_archived)
                                                <tr>
                                                    <td class="text-center">
                                                        <a href="{{ url('expedition/point/payment-order/'.$payment_order_archived->id.'/archived') }}"
