...
{
    public static function searchList($list_downpayment, $order_by, $order_type, $status, $date_from, $date_to, $search)
    {
        if ($status != 'all') {
    ...
    public static function create(Request $request, $formulir)
    {
        ...
        $downpayment->expedition_id = $request->input('expedition_id');
        $downpayment->expedition_order_id = $request->input('expedition_order_id') ? $request->input('expedition_order_id') : null;
